...
void CActor::g_cl_CheckControls(u32 mstate_wf, Fvector &vControlAccel, float &Jump, float dt)
{
	...
	if (pInput->GetControllerMode() && mstate_wf & mcAccel)
	{
...
bool CActor::CanMove()
{
	if( conditions().IsCantWalk() )
	{
		...
		{
			CurrentGameUI()->AddCustomStatic("cant_walk", true, 3.0f);
		}
		return false;
	}else
	if( conditions().IsCantWalkWeight() )
	{
		if(mstate_wishful&mcAnyMove)
		{
			CurrentGameUI()->AddCustomStatic("cant_walk_weight", true, 3.0f);
		}
		return false;
